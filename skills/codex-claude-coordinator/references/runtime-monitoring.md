...
- `preflight`：创建运行目录、锁和启动参数。
- `discover`：读文件、搜索和仓库调查。
- `plan`：任务列表或计划工具活动。
- `execute`：编辑、写文件、普通命令或实现子代理。
- `verify`：测试、lint、typecheck、build 等命令。
- `deliver`：最终报告、完成、阻塞或失败。

阶段根据事件和工具名称推断，适用于不同语言和构建系统，不表示模型的隐藏思维过程。

## 限制与退出码

- 默认总时限 3600 秒，空闲时限 900 秒，心跳 30 秒。
- 默认最多 4 个子代理、3 轮修复；任务契约和外部监控共同约束。
