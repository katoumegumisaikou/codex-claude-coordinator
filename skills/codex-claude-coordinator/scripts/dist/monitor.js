...
export function numberField(value, ...keys) {
    if (!isRecord(value))
        return undefined;
    for (const key of keys) {
        const candidate = value[key];
...
export function redactText(value, limit = 500) {
...
export function ensureRuntimeIgnored(projectRoot) {
...
function safeToolSummary(raw) {
    ...
    if (supplied)
        return redactText(supplied, 240);
    ...
    for (const key of ["command", "description", "query", "pattern", "file_path", "path"]) {
        ...
        if (typeof candidate === "string")
            return redactText(candidate.replace(/\s+/g, " "), 240);
    }
    return undefined;
}
export function normalizeHookEvent(raw, timestamp = new Date().toISOString()) {
    if (!isRecord(raw))
        return undefined;
...
export function normalizeStreamEvent(raw, timestamp = new Date().toISOString()) {
    if (!isRecord(raw))
        return undefined;
    ...
    if (type === "system") {
        ...
        if (eventType === "content_block_delta" || eventType === "message_delta" || eventType === "ping")
            return undefined;
    ...
    for (const key of ["type", "subtype", "session_id", "uuid", "is_error", "duration_ms", "duration_api_ms", "num_turns", "total_cost_usd"]) {
        if (key in raw)
            safeData[key] = sanitizeValue(raw[key]);
    }
    return {
        timestamp,
...
export function extractFinalReport(raw) {
    if (!isRecord(raw) || raw.type !== "result")
        return undefined;
    const result = raw.result;
    return typeof result === "string" ? result : undefined;
}
export function reportIndicatesBlocker(report) {
